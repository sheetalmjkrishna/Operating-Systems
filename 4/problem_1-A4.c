...
void lock(int id) {
  ...
  for (newId = 0; newId < threadCount; newId++) {
      ...
      while(Number[newId]!=0 && (Number[newId] < Number[id] || (Number[newId] == Number[id] && newId<id))) /*if token is lesser or token is the same but id is lesser, this thread has to wait*/
      {
      }
    }
}   
