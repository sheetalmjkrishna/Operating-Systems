...
ssize_t 
sleepy_read(struct file *filp, char __user *buf, size_t count,loff_t *f_pos)
{
  struct sleepy_dev *dev = (struct sleepy_dev *)filp->private_data;
  ssize_t retval = 0;
  int dev_num;
  dev_num = (int)iminor(filp->f_path.dentry->d_inode);
  printk("SLEEPY_READ DEVICE (%d): Process is waking everyone up. \n",dev_num);
  flag[dev_num] = 1;
  wake_up_interruptible(&my_queue[dev_num]);
...
ssize_t sleepy_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
  ...
  ssize_t retval = 0;
  int dev_num;
  ssize_t time_in_jiffies=0;
  dev_num = (int)iminor(filp->f_path.dentry->d_inode);
  if (mutex_lock_killable(&dev->sleepy_mutex))
    return -EINTR;
  if((count!=4)||(copy_from_user(&time_in_jiffies,buf,count)))
    return -EINVAL;
  if(time_in_jiffies<0)
    return retval;
  mutex_unlock(&dev->sleepy_mutex);
  retval = wait_event_interruptible_timeout(my_queue[dev_num], flag[dev_num]!=0, time_in_jiffies*HZ);
  flag[dev_num] = 0; 
     /*
	wait_event_interruptible_timeout returns:
	1) 0 if condition evaluates to false and timeout elapses
	2) 1 if condition evaluates to true and timeout elapses
	3)remaining jiffies if condition is evaluated to true and timeout hasn't elapsed
	4)-ERESTARTSYS if it was interupted by a signal

	if retval = 1, either timeout elapsed or there was 1 jiffy left. Assuming that if retval is 1, 
	timeout has elapsed.
  if(retval==1){
     retval=0;
  }
...
sleepy_init_module(void)
{
  ...
  dev_t dev = 0;
  for(i=0;i<10;i++){
    init_waitqueue_head(&my_queue[i]);
  }
  i=0;
